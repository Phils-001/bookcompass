...
<head>
    ...
    <style>
        ...
        .btn-export { background: #4CAF50; color: white; padding: 8px 15px; border: none; border-radius: 5px; cursor: pointer; margin-right: 10px; }
    ...
    <div class="container">
        <div class="card">
            ...
            <div style="display: flex; gap: 10px; margin-top: 10px;">
                ...
                {% else %}
                    <button style="background: #666; padding: 5px 15px; font-size: 12px; cursor: not-allowed;" disabled>🔒 Beta Access Only</button>
                {% endif %}
            </div>
            {% if remaining <= 0 %}
        ...
        <div class="card">
            ...
            <div style="display: flex; gap: 10px; margin-top: 10px;">
                <input type="text" id="referralLink" readonly value="https://bookcompass.app/signup?ref={{ username }}" style="flex: 1; background: #f5f5f5;">
                <button onclick="copyReferralLink()">📋 Copy</button>
            </div>
            <p style="font-size: 12px; color: #666; margin-top: 10px;">
                ✅ You've referred <strong id="referralCount">{{ referral_count }}</strong> friends
                {% if referral_credit > 0 %}
                    <br>🎁 You have <strong>{{ referral_credit }}</strong> months of 10% off credit!
                {% endif %}
            </p>
        </div>
    ...
    <script>
        ...
        function researchKeywords() {
            ...
            for(var i = 0; i < keywords.length; i++) {
                ...
                (function(index, kw) {
                    ...
                    .then(function(data) {
                        if(data.error) { 
                            errors.push({keyword: kw, error: data.error});
                        } else {
                            results.push(data);
                        }
                        processed++;
                    ...
                    .catch(function(error) {
                        errors.push({keyword: kw, error: error.message || 'Request failed'});
                        processed++;
                        if(processed === keywords.length) {
                            displayResults(results, errors);
                        }
        ...
        function copyAllToClipboard() {
            ...
            navigator.clipboard.writeText(text)
                .then(function() { alert('✅ Copied all results to clipboard!'); })
                .catch(function() {
